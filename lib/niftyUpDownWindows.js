...
/**
 * IST seconds where window W (1-based) **result** price is fixed: at window end (immediate).
 * Window 1: 09:15-09:30, Result at 09:30
 * Window 2: 09:30-09:45, Result at 09:45
 */
export function niftyResultSecForWindow(W, gameConfig = {}) {
  const { marketOpenSec } = getEffectiveNiftySessionBounds(gameConfig);
  const D = getNiftyRoundDurationSec(gameConfig);
  // Result is at the end of window W (immediate, no delay)
  return marketOpenSec + W * D;
}
