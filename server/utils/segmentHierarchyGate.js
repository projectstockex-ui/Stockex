...
export function isCryptoFutSegment(segmentName) {
  return String(segmentName || '').toUpperCase() === 'CRYPTOFUT';
}
...
export function validateEditorCanEnableLotQtyMode(parentSeg, field, requestingEnabled, editorRole, segmentName) {
  if (requestingEnabled !== true) return { allowed: true };
  if (editorRole === 'SUPER_ADMIN') return { allowed: true };

  if (field === 'enableLotSettings' || field === 'enableQuantitySettings') {
    if (isParentIntradayOnlyLocked(parentSeg) && !isCryptoFutSegment(segmentName)) {
      return {
        allowed: false,
...
export function enforceIntradayOnlyHierarchyOnSegment(parentSeg, childSeg, editorRole, segmentName) {
  ...
  if (!isCryptoFutSegment(segmentName)) {
    enforced.enableLotSettings = false;
    enforced.enableQuantitySettings = false;
  }

  return enforced;
}

export function validateSegmentIntradayOnlyHierarchy(parentSeg, segData, editorRole, segmentName) {
  ...
  for (const field of ['enableLotSettings', 'enableQuantitySettings']) {
    const lotQtyCheck = validateEditorCanEnableLotQtyMode(parentSeg, field, segData[field], editorRole, segmentName);
