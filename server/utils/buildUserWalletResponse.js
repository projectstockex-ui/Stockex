...
export async function buildUserWalletResponse(userId) {
  ...
  return {
    ...
    nseBseWallet: {
      ...
      ledgerReferenceBalance: ledgerStatus?.referenceBalance ?? walletUser.nseBseWallet?.ledgerReferenceBalance ?? nseBseBalance,
      ledgerClosePercent: ledgerStatus?.ledgerClosePercent ?? 90,
      ledgerMinEquityFloor: ledgerStatus?.minEquityFloor ?? 0,
      ledgerLossPercent: ledgerStatus?.lossPercent ?? 0,
