...
export function isCryptoFutSegment(segment) {
  return String(segment || '').toUpperCase() === 'CRYPTOFUT';
}
...
/** CRYPTOFUT keeps Lot/Qty toggles even when parent locked intraday-only (crypto is qty-based). */
export function canViewerManageLotQtySettings(viewerRole, parentBaseline, segment) {
  if (viewerRole === 'SUPER_ADMIN') return true;
  if (isCryptoFutSegment(segment)) return true;
  return !isParentIntradayOnlyLocked(parentBaseline, segment);
}
...
export function clampSegmentIntradayOnlyToParentBaseline(segDefs, parentBaseline, viewerRole) {
  ...
  for (const seg of Object.keys(out)) {
    ...
    const patch = { ...out[seg], defaultIntradayOnly: true };
    if (!isCryptoFutSegment(seg)) {
      patch.enableLotSettings = false;
      patch.enableQuantitySettings = false;
    }
    out[seg] = patch;
  }
